<?php
	...
	if ($SQLResult->num_rows < 1) {
	    ...
	    require('../template/head.php');
		echo "<DIV CLASS=\"alert alert-danger\" ROLE=\"alert\">User ".$UserDoingUpdateNetID." not found. Please <A HREF=\"../Users/UsersStudentCreateNew.php\">register</A> for access.</DIV>";
		goto CloseSQL;
	}
    ...
    if (isset($_GET["statussearch"]))
       	if (!ValidateField("statussearch", "RPASNVCD", 5))
    ...
    if ($SPNID != 0) {
		...
		//Make sure this request is for this user.
    ...
    echo "<FORM METHOD=\"POST\" ACTION=\"SPNReqStudentProcessForm.php\">";

	//The create/update button.
    if ($SPNID != 0)
	    echo "<P>Back to <A HREF=\"SPNReqStudentCreateList.php?&statuscheck=".$StatusCheck."&statussearch=".$StatusSearch."&semestercheck=".$SemesterCheck."&semestersearch=".$SemesterSearch."&yearsearch=".$YearSearch."&coursecheck=".$CourseCheck."&coursesearch=".urlencode($CourseSearch)."&sectioncheck=".$SectionCheck."&sectionsearch=".urlencode($SectionSearch)."&sort1=".urlencode($Sort1)."&sort2=".urlencode($Sort2)."\">previous search</A></P>";
    ...
    if ($SPNID != 0) {
	    ...
	    if ($SQLResult->num_rows > 0) {
		    ...
		    while($Fields = $SQLResult->fetch_row() ) {
                ...
                if ( ($Fields[4] != $ChangeUser) || ($Fields[1] != $ChangeTime) ) {
    ...
    //Additional notes.
    if ($SPNID == 0)
	    echo "<P><STRONG>Reason for Request</STRONG><BR>";
	else
	    echo "<P><STRONG>Additional Notes</STRONG><BR>";
	echo "<TEXTAREA NAME=\"newnotes\" ROWS=\"5\" COLS=\"80\"></TEXTAREA></P>";

    //The create/update button.
    if ($SPNID != 0) {

        //If this is an existing request, store the request ID in a hidden field.
	    echo "<INPUT TYPE=\"HIDDEN\" NAME=\"spnid\" VALUE=\"".$SPNID."\">\r\n";
	    echo "<P><INPUT TYPE=\"SUBMIT\" VALUE=\"Update Request\"></P>";
		if ($Status == "C")
			echo "<P><INPUT TYPE=\"SUBMIT\" NAME=\"uncancel\" VALUE=\"Un-Cancel Request\"></P>";
		if ($Status == "R")
			echo "<P><INPUT TYPE=\"SUBMIT\" NAME=\"cancel\" VALUE=\"Cancel Request\"></P>";
	}
    else
