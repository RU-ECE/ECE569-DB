<?php
	...
	$ChangeFlag |= UpdateField("HourRecords", $HID, $UserDoingUpdateUID, $CurrentTime, "UserID", $UserDoingUpdateUID);
	...
	if ($ChangeFlag == TRUE)
		echo "<DIV CLASS=\"alert alert-success\" ROLE=\"alert\"><A href=\"HoursStudentCreateForm.php?hid=".$HID."\">Office hour ".$HID."</A> successfully updated.";
	else
		echo "<DIV CLASS=\"alert alert-warning\" ROLE=\"alert\">Nothing changed for <A href=\"HoursStudentCreateForm.php?hid=".$HID."\">office hour ".$HID."</A>.";

    echo " Back to <A HREF=\"HoursStudentCreateList.php?statuscheck=".$StatusCheck."&statussearch=".urlencode($StatusSearch)."&semestercheck=".$SemesterCheck."&semestersearch=".urlencode($SemesterSearch)."&yearsearch=".urlencode($YearSearch)."&namecheck=".$NameCheck."&namesearch=".urlencode($NameSearch)."&coursecheck=".$CourseCheck."&coursesearch=".urlencode($CourseSearch)."&daycheck=".$DayCheck."&daysearch=".urlencode($DaySearch)."&sort1=".urlencode($Sort1)."&sort2=".urlencode($Sort2)."\">previous search</A>.</DIV>\r\n";
