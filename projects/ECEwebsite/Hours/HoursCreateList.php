<?php
    ...
    echo "<FORM METHOD=\"GET\" ACTION=\"HoursCreateList.php\">\r\n";
    ...
    echo "<TR><TD><INPUT TYPE=\"checkbox\" VALUE=\"checked\" NAME=\"namecheck\"".$NameCheck."> Show only hours for Name (first or last or partial) <INPUT TYPE=\"text\" NAME=\"namesearch\" VALUE=\"".$NameSearch."\" SIZE=\"20\" MAXLENGTH=\"50\"></TD></TR>\r\n";
    ...
    if ($NameCheck && $NameSearch)
	    $Conditions .= "AND ((FirstName LIKE '%".mysqli_real_escape_string($mysqli, $NameSearch)."%') OR (LastName LIKE '%".mysqli_real_escape_string($mysqli, $NameSearch)."%')) ";
    ...
    if ($SQLResult->num_rows > 0) {
        ...
        while($Fields = $SQLResult->fetch_assoc()) {
            echo "<TR>";
            echo "<TD><A HREF=\"HoursCreateForm.php?hid=".$Fields["HID"]."&statuscheck=".$StatusCheck."&statussearch=".urlencode($StatusSearch)."&semestercheck=".$SemesterCheck."&semestersearch=".urlencode($SemesterSearch)."&yearsearch=".urlencode($YearSearch)."&namecheck=".$NameCheck."&namesearch=".urlencode($NameSearch)."&coursecheck=".$CourseCheck."&coursesearch=".urlencode($CourseSearch)."&daycheck=".$DayCheck."&daysearch=".urlencode($DaySearch)."&sort1=".urlencode($Sort1)."&sort2=".urlencode($Sort2)."\">".$StatusList[$Fields["Status"]]."</A></TD>";
            echo "<TD>".$SemesterList[$Fields["Semester"]]." ".$Fields["Year"]."</TD>";
            echo "<TD>".$DayList[$Fields["Day"]]."</TD>";
            echo "<TD>".$CourseList[$Fields["CourseID"]]."</TD>";
//            echo "<TD><A HREF=\"../Users/UsersCreateForm.php?uid=".$Fields["UserID"]."\">".$Fields["FirstName"]." ".$Fields["LastName"]."</A></TD>";
