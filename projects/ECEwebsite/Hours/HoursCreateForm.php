<?php
    ...
    if ($HID != 0) {
		...
		$SQLQuery = "SELECT Hours.HID, Hours.CreateTime, Hour.Status, Hour.Semester, Hour.Year, Hour.UserID, Hour.CourseID, Hour.RoomID, Hour.Day, Hour.StartTime, Hour.StopTime, User.NetID FROM Hours
			LEFT JOIN
				(SELECT HID,
    ...
    echo "<FORM METHOD=\"POST\" ACTION=\"HoursProcessForm.php\">";

	//The create/update button.
    if ($HID != 0)
	    echo "<P><INPUT TYPE=\"SUBMIT\" VALUE=\"Update Office Hour\"> Back to <A HREF=\"HoursCreateList.php?statuscheck=".$StatusCheck."&statussearch=".urlencode($StatusSearch)."&semestercheck=".$SemesterCheck."&semestersearch=".urlencode($SemesterSearch)."&yearsearch=".urlencode($YearSearch)."&namecheck=".$NameCheck."&namesearch=".urlencode($NameSearch)."&coursecheck=".$CourseCheck."&coursesearch=".urlencode($CourseSearch)."&daycheck=".$DayCheck."&daysearch=".urlencode($DaySearch)."&sort1=".urlencode($Sort1)."&sort2=".urlencode($Sort2)."\">previous search</A></P>";
    else
